  ...
  addContent() {
    ...
    if (this.chapters.length > 0) {
      previousChapterButton =
        `<button class="previous-chapter-button control-button" aria-label="Previous Chapter"></button>`;
      nextChapterButton =
        `<button class="next-chapter-button control-button" aria-label="Next Chapter"></button>`;
    }
...
`;
    ...
    this.parts.buttonsMessage = this.shadowRoot.querySelector(
      ".buttons-message",
    );
    this.parts.buttonsRow = this.shadowRoot.querySelector(".buttons-row");
    this.parts.contentWarning = this.shadowRoot.querySelector(
      ".content-warning",
    );
    this.parts.fader = this.shadowRoot.querySelector(".fader");
    this.parts.fastForwardButton = this.shadowRoot.querySelector(
      ".fast-forward-button",
    );
    this.parts.fastForwardDisplay = this.shadowRoot.querySelector(
    ...
    if (this.chapters.length > 0) {
      this.parts.previousChapterButton = this.shadowRoot.querySelector(
        ".previous-chapter-button",
      );
      this.parts.nextChapterButton = this.shadowRoot.querySelector(
        ".next-chapter-button",
      );
    }
  }
  ...
  async getTitle() {
    ...
    if (this.attrs["advisory"] !== null) {
      contentAdvisory = `<div class="content-advisory">${
        this.attrs["advisory"]
      }</div>`;
    }
    if (this.attrs["title"] !== null) {
      this.parts.title.innerHTML = `<div>${
        this.attrs["title"]
      }</div>${contentAdvisory}`;
    } else {
      const url =
        `https://www.youtube.com/oembed?url=http://www.youtube.com/watch?v=${this.videoId}&format=json`;
      ...
      if (!response.ok) {
        ...
        this.parts.title.innerHTML =
          `<div>${json.title}</div>${contentAdvisory}`;
      }
    }
  }
  ...
  handleNextChapterButtonClick(event) {
    ...
    for (
      let checkIndex = 0;
      checkIndex < this.chapters.length;
      checkIndex += 1
    ) {
  ...
  handlePreviousChapterButtonClick(event) {
    const currentTime = this.player.getCurrentTime();
    for (
      let checkIndex = this.chapters.length - 1;
      checkIndex >= 0;
      checkIndex -= 1
    ) {
  ...
  handleSkipBackButtonClick(event) {
    this.player.seekTo(
      Math.max(
        0,
        this.player.getCurrentTime() - this.attrs["skip-back"],
      ),
    );
